...
if (!$dbi) {
    ...
    if (isset($pdfFileName) && isset($ID_PROCESO) && isset($pdfBase64)) {
        ...
        if ($result) {
            ...
            oci_rollback($dbi); // Revertir los cambios en caso de erro
...
$data = json_decode($json, true);
$ID_PROCESO = $data['ID_PROCESO'];
//echo("<script>console.log('si esta entrado en archivo ID PROCESO ES :".$ID_PROCESO ." ')</script>");
error_log($ID_PROCESO);
*/
?>
