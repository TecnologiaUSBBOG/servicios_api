...
// Primera inserción:
if (
    empty($id_sesion) || empty($ciclo) || empty($tipoTutoria) || empty($facultad) || empty($programa) || empty($curso) || empty($profesor) || empty($tematica) || empty($modalidad) || empty($metodologia) || empty($fechatutoria)
...
$insercion = "UPDATE ACADEMICO.GRUPOS_SESIONES_TUTORIAS SET FACULTAD ='$facultad', PROGRAMA='$programa',NOMBREDELCURSO='$curso',TEMATICA='$tematica', MODALIDAD='$modalidad', FECHATUTORIA=TO_DATE('$fechatutoria', 'DD/MM/YYYY HH24:MI'), LUGAR='$lugar' WHERE NUMEROSESION='$id_sesion' AND GRUPO='$id_grupo' ";
        ...
        if (is_array($estudiante) && isset($estudiante['DOCUMENTO_EST'])) {
            ...
            if ($Ejecutar_Consulta2 && $Ejecutar_Consulta1) {
                ...
                if ($Valor_NombEst && $Valor_CorreoEst) {
                    $asunto = "Reprogramación sesión de tutoría";
                    $text = "
                    <style>
                    #tableA{
                    ...
                    <p>Este mensaje es para confirmar su reprogramacion en la tutoría grupal de <b>$curso</b> con el profesor(a) <b>$Valor_NombProf</b>.</p>
